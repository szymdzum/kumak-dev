...
<TldrBox>
CLI tools used 13x fewer tokens than MCP for identical browser automation tasks. MCP returns full accessibility snapshots; CLI returns targeted queries.

Self-documenting CLI tools let agents learn through introspection. Unix composability beats protocol servers for extensibility.
</TldrBox>

Anthropic published an interesting observation about the [Model Context Protocol and code execution](https://www.anthropic.com/engineering/code-execution-with-mcp). They noted that executable code in the filesystem might be more efficient for AI agents than protocol servers. Isn't that essentially what CLI tools already are?

I decided to test this hypothesis by comparing two approaches to browser automation with AI agents:

- **CLI approach**: [bdg](https://github.com/szymdzum/browser-debugger-cli) - A browser debugger CLI I built
